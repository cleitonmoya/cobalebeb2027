...
# where m/g/tau are 1-based indices into method_grid/function_grid/Tt_grid
# (method_grid above already matches simulation_run.R's methods_grid_ref /
# calibration_run.R's method_grid index-for-index, "stan" included).
# seed_base here IS the seed of that (method, f, Tt, replica)'s production
# chain 1; chain k below reuses the existing seed_base + (k - 1)
# convention (same as calibration_run.R) to reproduce every one of that
# task's production chains exactly (see simulation_run.R's header comment
# for the full derivation and the collision-avoidance rationale against
# calibration's seeds).
#
