...
PATH_TSCOUNT <- file.path(RESULTS_DIR, "tscount_literature_benchmark.csv")
PATH_PREDICTIVE_PF <- file.path(RESULTS_DIR, "predictive_loglik_pf.csv")
...
load_data <- function() {
	...
	# ---- Predictive log-likelihood, all 7 methods: the 5 PoissonLTDM
	# samplers (predictive_loglik_pf.R, particle filter) + the 2 tscount
	# literature benchmarks (tscount_literature_benchmark.R -- INGARCH and
	# log-linear, both Poisson; the negative-binomial variants were dropped
	# from that script). Both scripts must have been run already. ----
	predictive_pf <- read.csv(PATH_PREDICTIVE_PF, stringsAsFactors = FALSE)
	tscount <- read.csv(PATH_TSCOUNT, stringsAsFactors = FALSE)
	...
	return(list(y = y, Tt = Tt, fit = fit, rhat_t = rhat_t, ess_raw = ess_raw, ess_sec = ess_sec,
				...
				summary = summary_csv, predictive_pf = predictive_pf, tscount = tscount))
}
...
# ---- Console summary: log_lik/log_cpo per method, Delta_max, and
# predictive log-likelihood / CPU time across all 7 methods -------------
#
# Printed only -- no LaTeX fragment is generated (dropped as unnecessary
# machinery; the manuscript's own numbers are copied by hand from this
# output instead).
#
...
print_real_data_results_table <- function(summary_df, delta_max_W1, delta_max_W2, delta_max, predictive_pf, tscount) {
	tbl <- summary_df %>%
		mutate(method_label = METHOD_LABELS[as.character(method)]) %>%
		arrange(match(method, METHOD_LEVELS))

	cat("\n=== log-likelihood / log-CPO, 5 PoissonLTDM methods (campy) ===\n")
	print(tbl[, c("method_label", "log_lik", "log_cpo")], row.names = FALSE)
	...
	cat(sprintf(
		"\nCross-method agreement: max_t Delta_max(theta_t1) = %s, max_t Delta_max(theta_t2) = %s, Delta_max(W1) = %s, Delta_max(W2) = %s\n",
		fmt_delta(max_delta_theta1), fmt_delta(max_delta_theta2), fmt_delta(delta_max_W1), fmt_delta(delta_max_W2)
	))

	# ---- Predictive log-likelihood + CPU time, all 7 methods: 5
	# PoissonLTDM samplers (predictive_loglik_pf.R, particle filter) + 2
	# tscount literature benchmarks (INGARCH, log-linear -- both Poisson;
	# no negative-binomial variants). total_time_mean (per-chain) is used
	# for the PoissonLTDM side to be closer, though still not directly
	# comparable, to tscount's single-fit cpu_time_mean_sec -- see
	# tscount_literature_benchmark.R's header note on why these two CPU-time
	# figures are not an apples-to-apples ESS/s-style comparison. ----
	poisson_ltdm_rows <- predictive_pf %>%
		left_join(summary_df[, c("method", "total_time_mean")], by = "method") %>%
		transmute(
			method_label = METHOD_LABELS[as.character(method)],
			predictive_loglik = predictive_loglik_mean,
			cpu_time_sec = total_time_mean
		)

	tscount_rows <- tscount %>%
		transmute(
			method_label = model,
			predictive_loglik = predictive_loglik,
			cpu_time_sec = cpu_time_mean_sec
		)

	combined <- rbind(poisson_ltdm_rows, tscount_rows)

	cat("\n=== Predictive log-likelihood + CPU time, all 7 methods (campy) ===\n")
...
print_real_data_results_table(data$summary, data$delta_max_W1, data$delta_max_W2, data$delta_max,
							   data$predictive_pf, data$tscount)
