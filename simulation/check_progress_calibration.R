# simulation/check_progress_calibration.R
#
...
# anything (CALIBRATION_PHASE_DEFS_ONLY suppresses calibration_phase.R's
...
# two phases differ in more than just names: calibration_phase.R's
...
source("calibration_phase.R", local = FALSE)
...
if (!dir.exists("registry_calibration")) {
    ...
    # loadRegistry() re-sources calibration_phase.R internally (same
    # mechanism as the initial source() above) -- but
    # CALIBRATION_PHASE_DEFS_ONLY was already destroyed by
    # calibration_phase.R's own rm() the first time it ran, so it must be
    # re-armed before EVERY loadRegistry() call, not just once at the top
    # of this script (same bug class documented in check_progress.R).
    safe_load_registry <- function(...) {
        assign("CALIBRATION_PHASE_DEFS_ONLY", TRUE, envir = .GlobalEnv)
        batchtools::loadRegistry(...)
    }

    reg <- safe_load_registry("registry_calibration", writeable = FALSE)
    ...
    if (nrow(expired_ids) > 0) {
        ...
        Sys.sleep(10)
        reg <- safe_load_registry("registry_calibration", writeable = FALSE)  # re-sync
