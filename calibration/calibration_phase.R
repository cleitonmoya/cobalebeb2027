...
grid_subset <- function(grid) subset(grid, N == 520000)
...
method_configs <- rbind(
    ...
    # N=520000/burnin=50000 (same ~9.6% burnin ratio as the other 4 rows):
    # a "curiosity" fifth point on the rhat-vs-N plateau, not meant for
    # production. Extrapolating a log-log fit of (rhat_max - 1) vs N over
    # the existing 4 points predicts ~1.011 here -- still just above the
    # 1.01 threshold, with ~628000 needed to actually cross it -- i.e.
    # this point is expected to land right at "still not converged, and
    # would need over half a million iterations to get there", which is
    # exactly the quantitative evidence the article wants for
    # amh_montoril's structural Tt=1600 mixing limitation. Cheap to run:
    # only 4 grid cells (constant/quadratic x Tt=200/1600, same as every
    # other config here), not the full production grid.
    data.frame(method = "amh_montoril",  N = 520000, burnin = 50000, K = NA, category = "medio"),
