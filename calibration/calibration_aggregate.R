# calibration/calibration_aggregate.R
#
...
# calibration_run.R: concurrent PBS jobs do not each write to the shared
...
# finished. Also needed for calibration_run.R's checkpoint: a
...
# configuration (see method_configs in calibration_run.R), and those
...
# calibration_run.R) turns the result into one summary.csv row per task.
...
# print_and_plot_diagnostics() from calibration_run.R -- sourcing it here
...
# CALIBRATION_PHASE_DEFS_ONLY <- TRUE first suppresses calibration_run.R's
...
if (length(rds_files) == 0) {
    stop(sprintf("No .rds files found in %s -- has calibration_run.R been run yet?", path_results))
}
...
for (i in seq_along(rds_files)) {
    ...
    # run_calibration_task() in calibration_run.R).
